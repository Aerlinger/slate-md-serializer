...
InlineLexer.prototype.parse = function (src) {
  var trail = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
  ...
  while (src) {
    ...
    if (cap = this.rules.strong.exec(src)) {
      src = src.substring(cap[0].length);
      var rendered = this.renderer.strong(this.parse(cap[2] || cap[1], trail.concat('strong')));
    ...
    if (cap = this.rules.em.exec(src)) {
      src = src.substring(cap[0].length);
      var _rendered = this.renderer.em(this.parse(cap[2] || cap[1], trail.concat('em')));
      this.applyTrailMarks(_rendered, trail, ['strong']);
      out.push(_rendered);
      continue;
    }
...
Renderer.prototype.groupTextInLeaves = function (childNode) {
  ...
  var output = node.reduce(function (acc, current) {
    if (current.text) {
      var previous = acc.slice(-1)[0];
      var previousMarks = previous && previous.object === 'text' ? (previous.marks || []).map(function (_ref) {
        var type = _ref.type;
        return type;
      }).join('') : null;
      var currentMarks = previousMarks !== null ? (current.marks || []).map(function (_ref2) {
        var type = _ref2.type;
        return type;
      }).join('') : null;
      ...
      return acc.concat(_this2.groupTextInLeaves(current));
    } else {
      acc.push(current);
      return acc;
    }
    ...
    case "list_item_start":
      {
        ...
        while (this.next().type !== "list_item_end") {
          _body3.push(this.token.type === "text" ? this.inline.parse(this.token.text) : this.tok());
        }
